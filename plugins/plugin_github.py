...
from taskforge.plugins import IntegrationPlugin, PluginHook, PluginMetadata
...
class GitHubIntegrationPlugin(IntegrationPlugin):
    ...
    def on_task_updated(self, task: Task, old_task: Task, user: User, **kwargs):
        ...
        try:
            ...
            if (
                ...
                and self.settings["auto_close_issues"]
            ):
                issue.edit(state="closed")
                issue.create_comment("Task completed in TaskForge ✅")

            # Reopen issue if task is no longer done
            elif task.status != TaskStatus.DONE and old_task.status == TaskStatus.DONE:
                ...
                try:
                    if "github_issue_id" in task.integration_data:
                        ...
                        issue.edit(
                            ...
                            labels=self._get_labels_for_task(task),
                            state=(
                                "closed" if task.status == TaskStatus.DONE else "open"
                            ),
                        )
                    else:
                        # Create new issue
    ...
    def _create_branch_for_task(self, repo, task: Task, issue):
        ...
        try:
            ...
            # Create branch name
            branch_name = (
                f"task/{issue.number}-{task.title.lower().replace(' ', '-')[:30]}"
            )
