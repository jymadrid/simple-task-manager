...
from taskforge.plugins import NotificationPlugin, PluginMetadata, PluginHook
...
class SlackIntegrationPlugin(NotificationPlugin):
    ...
    def _format_task_created_message(self, task: Task, user: User) -> str:
        ...
        user_mention = f"<@{user.username}>" if self.settings['mention_users'] else user.username
    ...
    def _send_slack_message(self, message: str, channel_or_project: str = None, urgent: bool = False):
    ...
    def create_task_from_slash_command(self, command_text: str, user_id: str) -> Dict[str, Any]:
        ...
        if not parts or parts[0] != 'create':
            return {'error': 'Invalid command. Use: /taskforge create "Title" [options]'}
