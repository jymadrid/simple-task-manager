...
from taskforge.storage.models import Base, TaskModel, ProjectModel, UserModel
...
class PostgreSQLStorage(StorageBackend):
    ...
    async def search_tasks(self, query: TaskQuery, user_id: str) -> List[Task]:
        ...
        async with self._get_session() as session:
            ...
            if query.priority:
                conditions.append(TaskModel.priority.in_([p.value for p in query.priority]))
    ...
    async def get_task_statistics(self, project_id: Optional[str] = None, 
        ...
        async with self._get_session() as session:
            ...
            # Completed tasks
            completed_query = base_query.where(TaskModel.status == TaskStatus.DONE.value)
            ...
            # In progress tasks
            in_progress_query = base_query.where(TaskModel.status == TaskStatus.IN_PROGRESS.value)
