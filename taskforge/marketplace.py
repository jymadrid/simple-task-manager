...
class PluginMarketplace:
    ...
    async def search_plugins(
        ...
        for listing in self.plugins.values():
            ...
            # Apply filters
            if query and query.lower() not in (listing.name + " " + listing.description).lower():
                continue
    ...
    async def get_plugin_details(self, plugin_id: str, include_reviews: bool = True) -> Dict[str, Any]:
    ...
    async def get_developer_analytics(self, developer_id: str) -> Dict[str, Any]:
        ...
        developer_plugins = [
            ...
            if p.author_email == developer.email
        ]
    ...
    async def _validate_plugin_file(self, plugin_file: bytes, metadata: PluginMetadata) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                with zipfile.ZipFile(temp_file_path, 'r') as zip_file:
                    ...
                    for file_name in file_list:
                        if any(file_name.lower().endswith(pattern) for pattern in dangerous_patterns):
                            validation_result["errors"].append(f"Potentially dangerous file: {file_name}")
                    ...
                    try:
                        ...
                        if file_metadata.get('version') != metadata.version:
                            validation_result["errors"].append("Version mismatch between file and submission")
                        
                        # Check name consistency
                        if file_metadata.get('name') != metadata.name:
                            validation_result["errors"].append("Name mismatch between file and submission")
                        
                    except (KeyError, json.JSONDecodeError) as e:
                        validation_result["errors"].append(f"Invalid metadata.json: {e}")
            ...
            if len(plugin_file) > 50 * 1024 * 1024:
                validation_result["errors"].append("Plugin file size exceeds 50MB limit")
    ...
    async def _security_scan_plugin(self, plugin_file: bytes) -> Dict[str, Any]:
        ...
        for dangerous_import in dangerous_imports:
            if dangerous_import in content:
                scan_result["issues"].append(f"Potentially dangerous code: {dangerous_import}")
...
class PluginAnalytics:
    ...
    async def get_download_trends(self, plugin_id: str, days: int = 30) -> Dict[str, Any]:
...
async def demo_plugin_marketplace():
    ...
    try:
        ...
        await marketplace.approve_plugin(slack_plugin_id, "reviewer-1", "Excellent plugin with comprehensive features")
    ...
    try:
        ...
        await marketplace.approve_plugin(analytics_plugin_id, "reviewer-1", "Great free analytics solution")
    ...
    try:
        ...
        if dev_analytics['plugins']:
            ...
            for plugin_stats in dev_analytics['plugins']:
                print(f"        - {plugin_stats['name']}: {plugin_stats['downloads']} downloads")
    ...
    if slack_plugin_id:
        try:
            plugin_details = await marketplace.get_plugin_details(slack_plugin_id, include_reviews=True)
            ...
            print(f"      Rating: {plugin_details['rating_average']}/5 ({plugin_details['rating_count']} reviews)")
            print(f"      Downloads: {plugin_details['downloads']}")
            print(f"      Developer: {plugin_details.get('developer', {}).get('display_name', 'Unknown')}")
            ...
            if plugin_details.get('recent_reviews'):
                ...
                for review in plugin_details['recent_reviews'][:2]:
                    ...
                    print(f"          \"{review['content'][:60]}...\" - {review['username']}")
