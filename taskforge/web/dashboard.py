...
def create_completion_trend_chart(tasks: List[Task]) -> go.Figure:
    ...
    if not completion_dates:
        # Return empty chart
        fig = go.Figure()
        fig.add_annotation(text="No completed tasks yet", 
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a page",
        ["Dashboard", "Tasks", "Projects", "Analytics", "Settings"]
    )
...
def render_dashboard_page():
    ...
    with col1:
        st.metric(
            label="Total Tasks",
            value=len(data['all_tasks']),
            delta=None
        )
    
    with col2:
        completed_count = len([t for t in data['all_tasks'] if t.status == TaskStatus.DONE])
        st.metric(
    ...
    with col3:
        in_progress_count = len([t for t in data['all_tasks'] if t.status == TaskStatus.IN_PROGRESS])
        st.metric(
            label="In Progress",
...
def render_tasks_page():
        ...
        with st.form("create_task_form"):
            ...
            if submitted and title:
                try:
                    ...
                    # Save task
                    created_task = asyncio.run(mgr.create_task(task, st.session_state.current_user))
    ...
    try:
        ...
        query = TaskQuery(
            ...
            priority=[TaskPriority(p) for p in priority_filter] if priority_filter else None,
