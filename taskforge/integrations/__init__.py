...
class DataExporter:
    ...
    async def export_tasks_to_json(self, project_id: Optional[str] = None, 
    ...
    async def export_tasks_to_csv(self, project_id: Optional[str] = None, 
                                user_id: Optional[str] = None) -> str:
        ...
        writer.writerow([
            ...
            'Created By', 'Assigned To', 'Project ID', 'Created At',
            'Due Date', 'Progress', 'Tags', 'Category'
        ])
    ...
    async def export_tasks_to_markdown(self, project_id: Optional[str] = None, 
                                     user_id: Optional[str] = None) -> str:
        ...
        for status, status_tasks in status_groups.items():
            md_content.append(f"## {status.replace('_', ' ').title()} ({len(status_tasks)})")
...
class DataImporter:
    ...
    async def import_tasks_from_json(self, json_data: str, user_id: str) -> Dict[str, Any]:
    ...
    async def import_tasks_from_csv(self, csv_data: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in csv_reader:
                try:
                    ...
                    if tags_str:
                        task_data['tags'] = set(tag.strip() for tag in tags_str.split(','))
    ...
    async def import_from_trello_backup(self, trello_json: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for card in cards:
                ...
                try:
                    ...
                    if card.get('due'):
                        try:
                            task.due_date = datetime.fromisoformat(card['due'].replace('Z', '+00:00'))
                        except ValueError:
                            pass
                    ...
                    if labels:
                        task.tags = {label['name'] for label in labels if label.get('name')}
    ...
    async def import_from_asana_csv(self, csv_data: str, user_id: str) -> Dict[str, Any]:
                    ...
                    for task_row in tasks_data:
                        try:
                            ...
                            task = Task(
                                ...
                                project_id=created_project.id,
                                created_by=user_id,
                                assigned_to=task_row.get('Assignee', '').strip() or user_id
                            )
                            ...
                            if due_date_str:
                                try:
                                    task.due_date = datetime.strptime(due_date_str, '%m/%d/%Y')
                                except ValueError:
                                    pass
                            ...
                            if tags_str:
                                task.tags = set(tag.strip() for tag in tags_str.split(','))
                            ...
                            errors.append(f"Task '{task_row.get('Name', 'Unknown')}': {str(e)}")
...
class IntegrationManager:
    ...
    async def sync_with_external_service(self, service_type: str, 
    ...
    async def _sync_with_github(self, config: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _sync_with_trello(self, config: Dict[str, Any], user_id: str) -> Dict[str, Any]:
