...
class SearchIndex:
    ...
    def __init__(self):
        ...
        self.field_values: Dict[str, Dict[str, Set[str]]] = defaultdict(lambda: defaultdict(set))
...
class SearchEngine:
                ...
                if doc_id.startswith('task_'):
                    ...
                    if doc:
                        ...
                        if task:
                            ...
                            if query.highlight:
                                result.highlights = self._generate_highlights(doc, query.text)
    ...
    async def search_users(self, query: SearchQuery, requester_id: str) -> SearchResults:
        """Search for users"""
        start_time = datetime.utcnow()
    ...
    async def suggest_completions(self, query: str, entity_type: str = 'task', limit: int = 10) -> List[str]:
    ...
    def _apply_sorting(self, results: List[SearchResult], sorts: List[SearchSort]) -> List[SearchResult]:
        ...
        def sort_key(result):
            ...
            for sort_spec in sorts:
                ...
                if value is None:
                    value = '' if sort_spec.field in ['title', 'name', 'description'] else 0
                keys.append(value if sort_spec.ascending else -value if isinstance(value, (int, float)) else value)
            return tuple(keys)
        
        return sorted(results, key=sort_key, reverse=not sorts[0].ascending)
    
    def _generate_highlights(self, document: Dict[str, Any], query: str) -> Dict[str, List[str]]:
        ...
        for field, value in document.items():
            ...
            if field_highlights:
                highlights[field] = field_highlights[:3]  # Limit to 3 highlights per field
