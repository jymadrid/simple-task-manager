...
class AnalyticsEngine:
    ...
    async def get_productivity_metrics(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        tasks_completed = len([t for t in tasks if t.status == TaskStatus.DONE and 
                              t.completed_at and t.completed_at >= start_date])
        
        # Daily productivity
        daily_productivity = self._calculate_daily_productivity(tasks, start_date, end_date)
        
        # Velocity (tasks completed per day)
        velocity = tasks_completed / days if days > 0 else 0.0
        
        # Focus score (percentage of high/critical priority tasks)
        high_priority_tasks = len([t for t in tasks if t.priority in [TaskPriority.HIGH, TaskPriority.CRITICAL]])
        ...
        consistency_score = 1.0 - (statistics.stdev(daily_completions) / max(statistics.mean(daily_completions), 1)) if len(daily_completions) > 1 else 1.0
    ...
    async def get_project_analytics(self, project_id: str) -> Dict[str, Any]:
        ...
        if project.start_date and project.end_date:
            ...
            time_progress = min(1.0, elapsed_duration / total_duration if total_duration > 0 else 0.0)
        else:
        ...
        # Burndown data
        burndown_data = await self._calculate_burndown(tasks, project.start_date, project.end_date)
        ...
        result = {
            ...
            'schedule_health': 'on_track' if task_progress >= time_progress else 'behind',
            'team_performance': team_performance,
            'risk_score': risk_score,
            'burndown_data': burndown_data,
            'estimated_completion': await self._estimate_completion_date(tasks, project)
        }
        
        self._cache_result(cache_key, result)
        return result
    
    async def get_team_analytics(self, team_members: List[str], 
        ...
        for user_id in team_members:
            ...
            individual_metrics[user_id] = {
                ...
                'tasks_completed': len([t for t in user_tasks if t.status == TaskStatus.DONE]),
                'avg_completion_time': await self._calculate_avg_completion_time(user_tasks),
        ...
        # Team velocity
        completed_this_week = len([t for t in team_tasks if 
                                 t.status == TaskStatus.DONE and 
                                 t.completed_at and 
                                 t.completed_at >= datetime.utcnow() - timedelta(days=7)])
        ...
        result = {
            ...
            'workload_distribution': await self._calculate_workload_distribution(individual_metrics)
        }
    ...
    async def get_time_series(self, metric_name: str, period: TimePeriod, 
                             start_date: datetime, end_date: datetime,
                             filters: Optional[Dict[str, Any]] = None) -> TimeSeries:
    ...
    async def _get_filtered_tasks(self, project_id: Optional[str] = None, 
                                 user_id: Optional[str] = None) -> List[Task]:
    ...
    async def _get_user_tasks_in_period(self, user_id: str, start_date: datetime, 
                                       end_date: datetime) -> List[Task]:
    ...
    def _calculate_monthly_trends(self, tasks: List[Task]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for i in range(12):
            ...
            created_count = len([t for t in tasks if month_start <= t.created_at <= month_end])
            completed_count = len([t for t in tasks if 
                                 t.status == TaskStatus.DONE and 
                                 t.completed_at and 
                                 month_start <= t.completed_at <= month_end])
    ...
    def _calculate_daily_productivity(self, tasks: List[Task], 
        ...
        while current_date <= end_date.date():
            ...
            completed = len([t for t in tasks if 
                           t.status == TaskStatus.DONE and 
                           t.completed_at and 
                           day_start <= t.completed_at < day_end])
    ...
    async def _calculate_project_risk(self, project: Project, tasks: List[Task]) -> float:
    ...
    async def _calculate_burndown(self, tasks: List[Task], 
        ...
        while current_date <= end_date.date():
            completed_by_date = len([t for t in tasks if 
                                   t.status == TaskStatus.DONE and 
                                   t.completed_at and 
                                   t.completed_at.date() <= current_date])
            ...
            burndown.append({
                ...
                'ideal_remaining': max(0, total_tasks - 
                                     (total_tasks * (current_date - start_date.date()).days / 
    ...
    async def _estimate_completion_date(self, tasks: List[Task], project: Project) -> Optional[str]:
        ...
        recent_completions = len([t for t in tasks if 
                                t.status == TaskStatus.DONE and 
                                t.completed_at and 
                                t.completed_at >= thirty_days_ago])
    ...
    async def _calculate_workload_distribution(self, individual_metrics: Dict[str, Any]) -> Dict[str, float]:
        """Calculate workload distribution fairness"""
        if not individual_metrics:
            return {}
        
        task_counts = [metrics['tasks_assigned'] for metrics in individual_metrics.values()]
        if not task_counts:
            return {}
        ...
        return {
            ...
            'max_tasks': max_tasks,
            'min_tasks': min_tasks,
            'distribution_ratio': max_tasks / min_tasks if min_tasks > 0 else float('inf'),
            'fairness_score': 1.0 - (max_tasks - min_tasks) / mean_tasks if mean_tasks > 0 else 1.0
        }
    ...
    async def _calculate_metric_for_period(self, metric_name: str, start: datetime, 
                                          end: datetime, filters: Optional[Dict[str, Any]]) -> float:
    ...
    async def _generate_task_summary_report(self, **kwargs) -> AnalyticsReport:
        ...
        metrics = [
            ...
            Metric("Average Completion Time", stats['average_completion_time'], MetricType.AVERAGE)
        ]
    ...
    async def _generate_productivity_report(self, user_id: str, days: int = 30, **kwargs) -> AnalyticsReport:
        ...
        metrics = [
            ...
            Metric("Tasks Completed", productivity['tasks_completed'], MetricType.COUNT),
    ...
    async def _generate_project_report(self, project_id: str, **kwargs) -> AnalyticsReport:
        """Generate project report"""
        analytics = await self.get_project_analytics(project_id)
        
        metrics = [
            Metric("Project Progress", analytics['task_progress'], MetricType.PERCENTAGE),
    ...
    async def _generate_team_report(self, team_members: List[str], **kwargs) -> AnalyticsReport:
        ...
        metrics = [
            ...
            Metric("Collaboration Score", analytics['collaboration_score'], MetricType.PERCENTAGE)
        ]
