...
class PluginConfig:
    ...
    enabled: bool = True
    directories: list = field(default_factory=lambda: ["./plugins", "~/.taskforge/plugins"])
...
class Config:
    ...
    def _update_from_dict(self, data: Dict[str, Any]):
        ...
        for key, value in data.items():
            if hasattr(self, key):
                ...
                if isinstance(attr, (DatabaseConfig, ServerConfig, SecurityConfig, 
                                   NotificationConfig, PluginConfig)):
    ...
    def _load_from_env(self):
        ...
        self.database.port = int(os.getenv("TASKFORGE_DB_PORT", str(self.database.port)))
        ...
        self.database.password = os.getenv("TASKFORGE_DB_PASSWORD", self.database.password)
        ...
        self.server.debug = os.getenv("TASKFORGE_SERVER_DEBUG", "false").lower() == "true"
        self.server.workers = int(os.getenv("TASKFORGE_WORKERS", str(self.server.workers)))
        
        # Security configuration
        self.security.secret_key = os.getenv("TASKFORGE_SECRET_KEY", self.security.secret_key)
        ...
        # Notification configuration
        self.notifications.smtp_host = os.getenv("SMTP_HOST", self.notifications.smtp_host)
        self.notifications.smtp_port = int(os.getenv("SMTP_PORT", str(self.notifications.smtp_port)))
        ...
        self.notifications.from_email = os.getenv("FROM_EMAIL", self.notifications.from_email)
    ...
    def save(self, config_file: str):
        ...
        config_data = {
            ...
            "database": {
                ...
                "url": self.database.url if not any([
                    self.database.username, self.database.password
                ]) else "",  # Don't save credentials in URL
