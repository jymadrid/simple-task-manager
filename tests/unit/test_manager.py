...
class TestTaskManager:
    ...
    @pytest.mark.asyncio
    async def test_create_task(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
    ...
    @pytest.mark.asyncio
    async def test_update_task(self, task_manager: TaskManager, sample_task: Task, sample_user: User):
        """Test task updates"""
        updates = {
        ...
        updated_task = await task_manager.update_task(sample_task.id, updates, sample_user.id)
    ...
    @pytest.mark.asyncio
    async def test_update_nonexistent_task(self, task_manager: TaskManager, sample_user: User):
        """Test updating non-existent task"""
        with pytest.raises(ValueError, match="not found"):
            await task_manager.update_task("nonexistent-id", {"title": "New Title"}, sample_user.id)
    
    @pytest.mark.asyncio
    async def test_delete_task(self, task_manager: TaskManager, sample_task: Task, sample_user: User):
        """Test task deletion"""
    ...
    @pytest.mark.asyncio
    async def test_delete_nonexistent_task(self, task_manager: TaskManager, sample_user: User):
        """Test deleting non-existent task"""
        success = await task_manager.delete_task("nonexistent-id", sample_user.id)
        assert not success
    
    @pytest.mark.asyncio
    async def test_search_tasks(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        """Test task search functionality"""
        # Create multiple test tasks
        tasks = await TestHelper.create_tasks(task_manager, sample_user.id, sample_project.id, 5)
        ...
        # Test status filter
        await task_manager.update_task(tasks[0].id, {"status": TaskStatus.DONE}, sample_user.id)
    ...
    @pytest.mark.asyncio
    async def test_get_overdue_tasks(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
    ...
    @pytest.mark.asyncio
    async def test_get_upcoming_tasks(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
    ...
    @pytest.mark.asyncio
    async def test_get_task_statistics(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        """Test task statistics retrieval"""
        # Create tasks with different statuses
        tasks = await TestHelper.create_tasks(task_manager, sample_user.id, sample_project.id, 10)
        
        # Update some task statuses
        await task_manager.update_task(tasks[0].id, {"status": TaskStatus.DONE}, sample_user.id)
        await task_manager.update_task(tasks[1].id, {"status": TaskStatus.DONE}, sample_user.id)
        await task_manager.update_task(tasks[2].id, {"status": TaskStatus.IN_PROGRESS}, sample_user.id)
        
        stats = await task_manager.get_task_statistics(sample_project.id, sample_user.id)
    ...
    @pytest.mark.asyncio
    async def test_bulk_update_tasks(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        ...
        updated_tasks = await task_manager.bulk_update_tasks(task_ids, updates, sample_user.id)
        
        assert len(updated_tasks) == 5
        assert all(task.priority == TaskPriority.HIGH for task in updated_tasks)
    
    @pytest.mark.asyncio
    async def test_archive_completed_tasks(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        ...
        await task_manager.update_task(created_task.id, {"status": TaskStatus.DONE}, sample_user.id)
        
        # Archive tasks older than 30 days
        archived_count = await task_manager.archive_completed_tasks(sample_project.id, 30)
        
        assert archived_count >= 0  # Depends on mock implementation
    
    @pytest.mark.asyncio
    async def test_dependency_validation(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        ...
        await task_manager.update_task(created_task2.id, {"dependencies": created_task2.dependencies}, sample_user.id)
        
        # Try to create cycle: task1 depends on task2
        created_task1.add_dependency(created_task2.id, "blocks")
        
        with pytest.raises(ValueError, match="cycle"):
            await task_manager.update_task(created_task1.id, {"dependencies": created_task1.dependencies}, sample_user.id)
    
    @pytest.mark.asyncio
    async def test_project_progress_update(self, task_manager: TaskManager, sample_user: User, sample_project: Project):
        """Test automatic project progress updates"""
        # Create tasks for project
        tasks = await TestHelper.create_tasks(task_manager, sample_user.id, sample_project.id, 4)
