...
class TestJSONStorage:
    ...
    async def test_task_crud_operations(self, storage):
        ...
        # Create a task
        task = Task(
            title="Test Task", description="A test task", priority=TaskPriority.HIGH
        )
    ...
    async def test_task_search(self, storage):
        ...
        tasks = [
            Task(
                title="Frontend Bug", priority=TaskPriority.HIGH, status=TaskStatus.TODO
            ),
            Task(
    ...
    async def test_bulk_operations(self, storage):
        ...
        tasks = [
            Task(title=f"Task {i}", priority=TaskPriority.MEDIUM) for i in range(5)
        ]
