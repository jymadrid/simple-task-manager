...
class TestTaskAPI:
    ...
    async def test_list_tasks_with_filters(self, api_client, auth_headers, monkeypatch):
        ...
        response = api_client.get(
            "/tasks?priority=high&status=todo&limit=10", 
    ...
    async def test_update_task(self, api_client, auth_headers, monkeypatch):
        ...
        task_id = "test-task-id"
        response = api_client.patch(f"/tasks/{task_id}", json=update_data, headers=auth_headers)
        assert response.status_code == 200
...
class TestStatisticsAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_productivity_metrics(self, api_client, auth_headers, monkeypatch):
