...
def override_dependencies(mock_task_manager):
    ...
    mock_user = User(id="user-test-id", username="testuser", email="test@example.com", password_hash="hash")
...
def test_get_user(mock_task_manager):
    mock_task_manager.get_user.return_value = User(id="user-id", username="test", email="t@e.com", password_hash="h")
...
def test_create_project(mock_task_manager):
    ...
    mock_task_manager.create_project.return_value = Project(id="proj-id", owner_id=mock_user.id, **project_data)
...
def test_create_task(mock_task_manager):
    ...
    mock_task_manager.get_project.return_value = mock_project
    mock_task_manager.create_task.return_value = Task(id="task-id", created_by=mock_user.id, assigned_to=mock_user.id, **task_data)
