...
async def sample_project(task_manager: TaskManager, sample_user: User) -> Project:
    ...
    project = Project(
        name="Test Project", description="A test project", owner_id=sample_user.id
    )
...
async def sample_task(
    task_manager: TaskManager, sample_user: User, sample_project: Project
) -> Task:
...
class TestHelper:
    ...
    async def create_tasks(
        manager: TaskManager, user_id: str, project_id: str, count: int = 10
    ) -> list[Task]:
