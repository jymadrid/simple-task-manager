...
class CustomNotificationPlugin(TaskPlugin):
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            config_schema={
                ...
                "priority_threshold": {"type": "string", "enum": ["low", "medium", "high"], "default": "medium"},
    ...
    async def on_task_overdue(self, task: Task, user: User, **kwargs):
        ...
        logger.warning(f"⚠️ Overdue notification sent: {task.title} ({overdue_days} days)")
    
    @PluginHook('project_milestone_reached')
    async def on_project_milestone_reached(self, project: Project, milestone: str, user: User, **kwargs):
...
class GitHubIntegrationPlugin(TaskPlugin):
    ...
    async def on_task_created(self, task: Task, user: User, **kwargs):
        ...
        logger.info(f"🐙 Created GitHub issue #{issue_data['number']} for task: {task.title}")
    
    @PluginHook('task_status_changed')
    async def on_task_status_changed(self, task: Task, old_status: TaskStatus, new_status: TaskStatus, user: User, **kwargs):
        """Update GitHub issue when task status changes."""
        github_issue_id = task.metadata.get("github_issue_id") if task.metadata else None
        ...
        if new_status == TaskStatus.COMPLETED:
            ...
            logger.info(f"🐙 Closed GitHub issue #{github_issue_id} for completed task: {task.title}")
            ...
            logger.info(f"🐙 Updated GitHub issue #{github_issue_id} - task in progress")
    ...
    async def _create_github_issue(self, repo: str, title: str, description: str, labels: List[str], assignees: List[str]) -> Dict[str, Any]:
...
class ProjectAnalyticsPlugin(TaskPlugin):
    ...
    async def on_task_status_changed(self, task: Task, old_status: TaskStatus, new_status: TaskStatus, user: User, **kwargs):
        ...
        if new_status == TaskStatus.IN_PROGRESS:
            ...
            self.metrics_data["task_completion_times"].append({
                ...
                "actual_vs_estimate": (completion_time.total_seconds() / 3600) / (task.estimated_hours or 1)
            })
            del self.start_times[task.id]
            logger.info(f"📊 Recorded completion time for task: {task.title} ({completion_time})")
    ...
    def generate_productivity_report(self, user_id: str = None, project_id: str = None) -> Dict[str, Any]:
        ...
        # Calculate metrics
        avg_completion_time = sum(d["completion_time_hours"] for d in completion_data) / len(completion_data)
        total_tasks_completed = len(completion_data)
        
        # Estimate accuracy
        estimates_vs_actual = [d["actual_vs_estimate"] for d in completion_data if d["estimated_hours"] > 0]
        avg_estimate_accuracy = sum(estimates_vs_actual) / len(estimates_vs_actual) if estimates_vs_actual else 0
        ...
        report = {
            "summary": {
                ...
                "productivity_trend": "improving" if avg_estimate_accuracy < 1.2 else "needs_attention"
            },
    ...
    def _generate_recommendations(self, completion_data: List[Dict[str, Any]]) -> List[str]:
        ...
        avg_time = sum(d["completion_time_hours"] for d in completion_data) / len(completion_data)
        
        if avg_time > 8:
            recommendations.append("Consider breaking down large tasks into smaller, manageable chunks")
        
        # Analyze estimate accuracy
        estimates = [d["actual_vs_estimate"] for d in completion_data if d["estimated_hours"] > 0]
        if estimates:
            avg_accuracy = sum(estimates) / len(estimates)
            if avg_accuracy > 1.5:
                recommendations.append("Your time estimates tend to be optimistic - consider adding buffer time")
            elif avg_accuracy < 0.8:
                recommendations.append("Your time estimates are often too high - you're more efficient than you think!")
        
        # Priority analysis
        high_priority_count = len([d for d in completion_data if d["priority"] == "high"])
        total_count = len(completion_data)
        
        if high_priority_count / total_count > 0.7:
            recommendations.append("You're handling many high-priority tasks - consider delegating or reprioritizing")
...
async def plugin_development_demo():
    ...
    github_plugin.configure_repo_mapping(
        created_project.id,
        "taskforge-community/plugin-ecosystem"
    )
    ...
    await github_plugin.on_task_status_changed(first_task, old_status, TaskStatus.IN_PROGRESS, user)
    await analytics_plugin.on_task_status_changed(first_task, old_status, TaskStatus.IN_PROGRESS, user)
    ...
    await github_plugin.on_task_status_changed(first_task, old_status, TaskStatus.COMPLETED, user)
    await analytics_plugin.on_task_status_changed(first_task, old_status, TaskStatus.COMPLETED, user)
    ...
    print(f"      Average completion time: {report['summary']['average_completion_time_hours']} hours")
    ...
    recent_notifications = notification_plugin.notifications[-3:]  # Last 3 notifications
