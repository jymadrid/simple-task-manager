...
class PerformanceBenchmark:
    ...
    async def benchmark_task_creation(self, num_tasks: int = 1000) -> BenchmarkResult:
        ...
        print(f"   ✅ Task creation benchmark complete: {result.operations_per_second:.1f} ops/sec")
        return result
    ...
    async def benchmark_task_queries(self, num_queries: int = 500) -> BenchmarkResult:
        ...
        self.results.append(result)
        print(f"   ✅ Query benchmark complete: {result.operations_per_second:.1f} queries/sec")
        return result
    ...
    async def benchmark_task_updates(self, num_updates: int = 500) -> BenchmarkResult:
            ...
            try:
                ...
                update_operations = [
                    ...
                    lambda: self.manager.update_task(
                        task.id, {"priority": random.choice(list(TaskPriority))}, user_id
                    )
                ]
        ...
        self.results.append(result)
        print(f"   ✅ Update benchmark complete: {result.operations_per_second:.1f} updates/sec")
        return result
    
    async def benchmark_concurrent_operations(self, num_concurrent_users: int = 10, operations_per_user: int = 50) -> BenchmarkResult:
        """Benchmark concurrent operations from multiple users."""
        print(f"👥 Benchmarking concurrent operations ({num_concurrent_users} users, {operations_per_user} ops each)")
        ...
        result = BenchmarkResult(
            ...
            total_time_seconds=total_time,
            operations_per_second=len(all_latencies) / total_time if total_time > 0 else 0,
            average_latency_ms=stats["avg"],
            min_latency_ms=stats["min"],
            max_latency_ms=stats["max"],
            p95_latency_ms=stats["p95"],
        ...
        self.results.append(result)
        print(f"   ✅ Concurrent benchmark complete: {result.operations_per_second:.1f} ops/sec")
        return result
    
    async def benchmark_large_dataset_operations(self, dataset_size: int = 10000) -> BenchmarkResult:
        ...
        for batch_start in range(0, dataset_size, batch_size):
            ...
            for i in range(batch_start, batch_end):
                task = Task(
                    ...
                    tags=[f"large-dataset", f"batch-{i // batch_size}", f"category-{i % 10}"],
        ...
        operations = [
            ...
            ("Search high priority", lambda: self.manager.search_tasks(
                self.manager.TaskQuery(priority=[TaskPriority.HIGH]), user_id
            )),
            ("Search by tag", lambda: self.manager.search_tasks(
                self.manager.TaskQuery(tags=["large-dataset"]), user_id
            )),
            ("Search by project", lambda: self.manager.search_tasks(
        ...
        for op_name, op_func in operations:
            ...
            print(f"      {op_name}: {op_time:.3f}s ({len(results) if results else 0} results)")
    ...
    async def create_sample_data(self, num_users: int, num_projects: int, num_tasks: int):
        """Create sample data for benchmarking."""
        print(f"📊 Creating sample data ({num_users} users, {num_projects} projects, {num_tasks} tasks)")
        ...
        for i in range(num_projects):
            ...
            created_project = await self.manager.create_project(project, self.test_data["users"][0])
            self.test_data["projects"].append(created_project)
        
        # Create tasks
        for i in range(num_tasks):
            user_id = random.choice(self.test_data["users"])
            project = random.choice(self.test_data["projects"]) if self.test_data["projects"] else None
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if avg_ops_per_sec < 100:
            recommendations.append("Consider optimizing database queries or using connection pooling")
        
        # Check for high latency
        high_latency_tests = [r for r in self.results if r.p95_latency_ms > 1000]
        if high_latency_tests:
            recommendations.append(f"High latency detected in {len(high_latency_tests)} tests - investigate bottlenecks")
        
        # Check for errors
        error_tests = [r for r in self.results if r.error_count > 0]
        if error_tests:
            recommendations.append(f"Errors detected in {len(error_tests)} tests - review error handling")
        
        # Storage-specific recommendations
        if self.storage_type == "json":
            recommendations.append("Consider upgrading to PostgreSQL for better performance at scale")
    ...
    def print_summary(self):
        ...
        print(f"{'Test Name':<25} {'Ops/sec':<12} {'Avg Latency':<15} {'P95 Latency':<15} {'Errors':<8}")
        print("-" * 75)
        ...
        print(f"   Fastest Test: {fastest_test.test_name} ({fastest_test.operations_per_second:.1f} ops/sec)")
        print(f"   Slowest Test: {slowest_test.test_name} ({slowest_test.operations_per_second:.1f} ops/sec)")
        
        # Recommendations
        print("\n💡 Recommendations:")
        if avg_ops_per_sec > 1000:
            print("   ✅ Excellent performance! Your TaskForge setup is well optimized.")
        elif avg_ops_per_sec > 500:
            print("   👍 Good performance. Consider monitoring for potential bottlenecks.")
        elif avg_ops_per_sec > 100:
            print("   ⚠️  Moderate performance. Consider database optimization or hardware upgrades.")
        else:
            print("   🚨 Performance issues detected. Review database configuration and system resources.")
        
        if self.storage_type == "json" and total_ops > 10000:
            print("   💾 Consider upgrading to PostgreSQL for better performance with large datasets.")
        
        error_count = sum(r.error_count for r in self.results)
        if error_count > 0:
            print(f"   ❌ {error_count} errors detected across tests. Review error logs.")
        else:
    ...
    try:
        ...
        for storage_type, result in results.items():
            if "error" in result:
                ...
                print(f"✅ {storage_type}: {summary['total_tests']} tests, "
