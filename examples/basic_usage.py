...
async def project_management_example():
    ...
    completed_tasks = [
        t for t in project_tasks_list if t.status == TaskStatus.COMPLETED
    ]
    in_progress_tasks = [
        t for t in project_tasks_list if t.status == TaskStatus.IN_PROGRESS
    ]
...
async def advanced_querying_example():
    ...
    sample_tasks = [
        Task(
            title="Fix critical bug", priority=TaskPriority.HIGH, tags=["bug", "urgent"]
        ),
        Task(
    ...
    upcoming_query = TaskQuery(
        due_date_from=datetime.now(), due_date_to=datetime.now() + timedelta(days=3)
    )
    upcoming_tasks = await manager.search_tasks(upcoming_query, user_id)
    print(f"   📅 Tasks due in next 3 days: {len(upcoming_tasks)}")
    for task in upcoming_tasks:
        print(
            f"      - {task.title} (due: {task.due_date.strftime('%Y-%m-%d') if task.due_date else 'No due date'})"
        )
    ...
    print(
        f"   🎯 High/Medium priority feature/bug/security tasks: {len(complex_results)}"
    )
...
async def collaboration_example():
    ...
    await manager.update_task_status(
        alice_task.id, TaskStatus.IN_PROGRESS, team_members[0]
    )
    await manager.add_task_note(
        alice_task.id,
    ...
    await manager.update_task_status(
        charlie_task.id, TaskStatus.COMPLETED, team_members[2]
    )
    await manager.add_task_note(
        charlie_task.id,
    ...
    all_project_tasks = await manager.get_project_tasks(
        team_project.id, team_members[0]
    )
    ...
    for member in team_members:
        ...
        in_progress = len(
            [t for t in member_tasks if t.status == TaskStatus.IN_PROGRESS]
        )
    ...
    for member, stats in team_stats.items():
        print(
            f"   👤 {member.title()}: {stats['completed']}/{stats['total']} tasks "
    ...
    completed_tasks = len(
        [t for t in all_project_tasks if t.status == TaskStatus.COMPLETED]
    )
...
async def automation_example():
    ...
    for i, task in enumerate(dependency_tasks):
        ...
        if i > 0:
            await manager.add_task_dependency(
                created_task.id, created_dep_tasks[i - 1].id, user_id
            )
            print(
                f"   🔗 {created_task.title} depends on: {created_dep_tasks[i-1].title}"
            )
        else:
    ...
    print(
        f"   ⚡ Total automated workflow items: {3 + len(created_dep_tasks) + len(auto_tasks)}"
    )
...
async def main():
    ...
    try:
        ...
        print(
            f"🤖 Automation features: {len(automation_results['recurring']) + len(automation_results['dependencies']) + len(automation_results['automated'])} total items"
        )
